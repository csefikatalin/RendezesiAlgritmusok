...
function indit() {
    ...
    var d = new Date();
    var kezdo = d.getTime();
    kiir(buborSort(tomb, 0, tomb.length - 1, 0));
    var g = new Date();
    var vege = g.getTime();
    var ido = vege - kezdo;
...
function statisztikahoz() {
    ...
    for (var i = 5; i <= 101; i += 10) {
        ...
        let d = new Date();
        let kezdo = d.getTime();
        buborSort(tomb, 0, tomb.length - 1, 0);
        let g = new Date();
        let vege = g.getTime();
        let ido = vege - kezdo;
...
console.log(statido);
        var options = {
            title: 'Buborékos rendezési idők alakulása az elemszám függvényében',
            curveType: 'function',
...
function diagramdb() {
    ...
    function drawChart() {
        ...
        var options = {
            title: 'Buborékos rendezés cserék számának alakulása az elemszám függvényében',
            curveType: 'function',
            legend: {position: 'bottom'}
        };
